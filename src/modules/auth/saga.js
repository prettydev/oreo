...
import * as smsOtp from '../../utils/smsOtp'
...
function* signInWithOtp({ payload }) {
  try {
    const language = yield select(languageSelector);
    const data = yield call(smsOtp.login, payload);
...
function* signUpWithOtp({ payload }) {
  try {
    const language = yield select(languageSelector);
    const data = yield call(smsOtp.register, payload);
    const { token, user, success } = data;
    if (success) {
      yield call(showMessage, {
        message: languages[language].notifications.text_create_user_success,
...
export default function* authSaga() {
  ...
  yield takeEvery(Actions.SIGN_IN_WITH_OTP, signInWithOtp);
  yield takeEvery(Actions.SIGN_UP_WITH_OTP, signUpWithOtp);
