...
const ShippingMethod = ({ list, total, tax, currency, title, feeShipping, freeShippingTax}) => {
  ...
  return (
    <ContainerView title={title} subTitle={subTitle}>
      <Text colorSecondary style={styles.text}>
        {feeShipping}: {currencyFormatter(total, currency)}
      </Text>
      {isShippingTax && (
        <Text colorSecondary style={styles.text}>
          {freeShippingTax}: {currencyFormatter(tax, currency)}
        </Text>
      )}
    </ContainerView>
  )
