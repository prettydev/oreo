...
const OrderItem = ({data, navigation, style, notif, ...rest}) => {
  ...
  const onPress = () =>
    !notif && navigation.navigate(profileStack.order_detail, {order: data});
        ...
        <Row style={styles.row}>
          <Col style={styles.col}>
            ...
            <Text style={styles.textInfo} h6 colorThird>
              {!notif && t('profile:text_code_order', {code: data.number})}
              {!notif && '\n'}
              {t('profile:text_time_order', {time: strDate(data.date_created)})}
              {!notif && '\n'}
              {!notif && t('profile:text_status_order')}
              {!notif && (
